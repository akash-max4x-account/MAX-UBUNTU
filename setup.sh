...
setup_vnc() {
	...
	if [[ -d "$CURR_DIR/distro" ]] && [[ -e "$CURR_DIR/distro/vncstop" ]]; then
		...
		downloader "$CURR_DIR/vncstop" "https://raw.githubusercontent.com/akash.max4x-1/MAX-UBUNTU/master/distro/vncstop"
...
permission() {
	...
	if [[ -e "$PREFIX/bin/ubuntu" ]]; then
		...
		cat <<- EOF
			...
			${R} [${W}-${R}]${G} If you Want to Use UBUNTU in MAX then ,
